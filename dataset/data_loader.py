...
def Dataset(start_signal =START_SIGNAL, data_path=DATA_PATH,augment=True):
    ...
    X_right = np.array(pd.read_csv(data_path+'/'+'right_profiles.csv').drop('Unnamed: 0',axis=1))
    X_left = np.array(pd.read_csv(data_path+'/'+'left_profiles.csv').drop('Unnamed: 0',axis=1))
    Y = np.array(pd.read_csv(data_path+'/'+'target_scans.csv').drop('Unnamed: 0',axis=1))
    print('Preprocessing the dataset...\n')
    X_right = trim_X(X_right)
    X_left = trim_X(X_left)
    Y = trim_signal(first_peak_vector(Y),Y,start_signal)
    if augment==True:
        ...
        Y_bonus = trim_signal(first_peak_vector(Y_bonus),Y_bonus,start_signal)
    ...
    res = {'RNN data':X_rnn, 'Full Model Target': Y, 'Right Profile Data': X_right, 'Left Profile Data': X_left,
           'Main Peak Data':main_peak, 'Augmented Data': augment}
    return res
